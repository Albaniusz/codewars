...
public class DragonsCurveTest {
    ...
    public void testCreateCurve() throws Exception {
        Assert.assertEquals("Proof all chars are ints: ", 'a', 97);
        DragonsCurve testee = new DragonsCurve();
        tests.forEach((i, s) -> Assert.assertEquals("Curve size " + i + " ", s, testee.createCurve(i)));
    }
    ...
    public void testMapFunction() throws Exception {
        ...
        inOut.forEach((i, o) -> {
            ...
            Assert.assertEquals("Input to map function " + i + " (char '" + (char) i.intValue() + "') response ", o, res);
        });
    }
    ...
    public void testHowMany() throws Exception {
        ...
        for (int i = 0; i < curves.size(); i++) { //Some things are still better the old-fashioned way
            ...
            Assert.assertEquals("Number of F in " + p + " ", fCounts.get(i).longValue(), testee.howMany('F', p));
            Assert.assertEquals("Number of R in " + p + " ", rCounts.get(i).longValue(), testee.howMany('R', p));
            Assert.assertEquals("Number of L in " + p + " ", lCounts.get(i).longValue(), testee.howMany('L', p));
        }
    }
}
