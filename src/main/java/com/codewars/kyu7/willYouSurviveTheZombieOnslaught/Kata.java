...
public class Kata {
    ...
    private final static String MSG_FINAL = "You shot all %d zombies.";

    private final static String MSG_OVERWHELMED = "You shot %d zombies before being eaten: overwhelmed.";

    private final static String MSG_OUT_OF_AMMO = "You shot %d zombies before being eaten: ran out of ammo.";
    ...
    public static String zombieShootout(int zombies, int range, int ammo) {
        ...
        while (true) {
            if (zombies <= 0) {
                result = msg(MSG_FINAL, zCounter);
                break;
            } else {
                if (range <= 0) {
                    result = msg(MSG_OVERWHELMED, zCounter);
                    break;
                }
                if (ammo <= 0) {
                    result = msg(MSG_OUT_OF_AMMO, zCounter);
                    break;
                }
            }
    ...
    private static String msg(String msg, int counter) {
        return String.format(msg, counter);
    }
}
